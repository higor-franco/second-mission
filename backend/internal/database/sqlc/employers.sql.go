...
const listCandidatesForListing = `-- name: ListCandidatesForListing :many
SELECT
    va.id AS application_id, va.status, va.match_score, va.created_at AS applied_at,
    va.updated_at,
    v.id AS veteran_id, v.name, v.mos_code, v.rank, v.years_of_service,
...
type ListCandidatesForListingParams struct {
	ID         int32       `json:"id"`
	EmployerID pgtype.Int4 `json:"employer_id"`
}

type ListCandidatesForListingRow struct {
	ApplicationID   int32              `json:"application_id"`
	Status          string             `json:"status"`
	MatchScore      int32              `json:"match_score"`
	AppliedAt       pgtype.Timestamptz `json:"applied_at"`
...
func (q *Queries) ListCandidatesForListing(ctx context.Context, arg ListCandidatesForListingParams) ([]ListCandidatesForListingRow, error) {
	rows, err := q.db.Query(ctx, listCandidatesForListing, arg.ID, arg.EmployerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListCandidatesForListingRow
	for rows.Next() {
		var i ListCandidatesForListingRow
		if err := rows.Scan(
			&i.ApplicationID,
			&i.Status,
			&i.MatchScore,
